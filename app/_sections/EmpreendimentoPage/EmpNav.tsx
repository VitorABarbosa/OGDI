...
export function EmpNav({
  ...
  return (
    <nav
      ...
      className="sticky top-[56px] z-[60] border-b border-[color:var(--line)] bg-white/[.9] backdrop-blur-[18px] backdrop-saturate-[1.6]"
    >
      <div className="mx-auto flex h-[clamp(50px,5vw,60px)] max-w-[1440px] items-center justify-between gap-6 px-pad-x">
        <div className="flex shrink-0 items-baseline gap-2.5">
          <b className="font-sans text-[14px] font-semibold tracking-[-.01em] text-ink">{name}</b>
        ...
        <div className="flex min-w-0 items-center gap-[clamp(14px,1.7vw,30px)] overflow-x-auto [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {visible.map((it) => (
            <a
              ...
              className={cn(
                "shrink-0 whitespace-nowrap py-1 font-sans text-[12px] tracking-[.04em] transition-colors duration-200",
                active === it.id ? "text-ink" : "text-ink-3 hover:text-ink",
              )}
            >
              {it.label}
            </a>
